...
详见 `doc/Sprint0_代码现状对照表.md`。MVP 范围为 Phase 1–7：

| 状态 | Phase |
|------|-------|
| ✅ 完成 | Phase 0（骨架）、Phase 2–3（项目管理、对话历史）、Phase 4（知识库索引）、Phase 6（企业事实抽取）、Phase 7（文章生成 MVP）、Phase 8（Assistant Runtime）、Phase 9（Agent-first Task Runtime + Skill 编排） |
| ⚠️ 骨架 | Phase 1（仪表盘）、Phase 5（Hybrid Retrieval）、Phase 10（发布/可见性） |
| ❌ 未实现 | Phase 11（反思学习）、Phase 12（测试/打包） |

Phase 6–9 已交付：企业资料 → 事实抽取与确认 → 问题池 → 信源发现 → 文章生成 → Claim/GEO 审核 → Agent-first Runtime 编排的最小闭环已跑通。下一优先级为 Phase 10（发布计划与豆包可见性检测），其 Agent 侧暂停卡片（`publish.plan`）已在 Phase 9 runtime 中就绪，缺 publish_records 持久化与 VisibilityChecker 真实接入。
