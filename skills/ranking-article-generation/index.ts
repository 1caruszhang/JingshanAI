...
// NOTE(#64): ranking IPC 迁移 deferred until #63 lands (tool_call loop wiring
// into mdDrivenRunner). title IPC 已在 #64 迁移完成；ranking 因依赖
// tool_call 循环（artifact/entries/claims side-effects），其 IPC 迁移与
// legacy 代码删除待 #63 合入后作为 follow-up 进行。在此之前
// articleGenerationService.ts 仍调用 generateRankingArticle。
// The md-driven validate layer above is independently testable and does not
// depend on these. Do NOT delete until the #63 follow-up.

// TODO(#63 follow-up): remove once ranking IPC path migrated to md-driven
...
// TODO(#63 follow-up): remove once ranking IPC path migrated (duplicate of ragService.formatEvidence)
...
// TODO(#63 follow-up): remove once ranking IPC path migrated
